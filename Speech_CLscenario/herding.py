...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 27 09:54:34 2022
...
def herd_random(x: np.ndarray, y: np.ndarray, t: np.ndarray, transcripts, z: Any, nb_per_class: int, splitting_crit) -> np.ndarray:
    ...
    if splitting_crit is None:
        yy = np.unique(y)
    else:
        if splitting_crit == "scenario":
            yy = np.unique(y[:,0])
        else:
            yy = np.unique(y[:,1])
    ...
    #for class_id in np.unique(y):
        ...
        if splitting_crit is None:
            ...
            if splitting_crit == "scenario":
                class_indexes = np.where(y[:,0] == class_id)[0]
            else:
                class_indexes = np.where(y[:,1] == class_id)[0]
    ...
    selected_transcripts = temp_transc[indexes]
    return x[indexes], y[indexes], t[indexes], list(selected_transcripts)
